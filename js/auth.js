...
import { doc, setDoc, getDoc, updateDoc, increment } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    avatarOptions.forEach(opt => opt.onclick = () => {
    ...
    document.getElementById("createRoomBtn").onclick = async () => {
        ...
        const p = JSON.parse(localStorage.getItem("playerProfile"));
        if (p.rating < 50) return alert("You need at least 50 points to bet!");

        await set(ref(database, "rooms/" + code), {
            board: Array(9).fill(""),
            turn: "X", status: "waiting", players: { X: p }
        });
